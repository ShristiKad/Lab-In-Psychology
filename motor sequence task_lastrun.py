...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\shreya kad\\Desktop\\ahmedabad university\\Lab In Psychology\\Motor Sequence Task\\motor sequence task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "random_routine_" ---
    fixation2 = visual.ShapeStim(
        ...
        opacity=None, depth=0.0, interpolate=True)
    line1_1 = visual.Rect(
        win=win, name='line1_1',units='pix', 
        width=(100, 10)[0], height=(100, 10)[1],
        ori=90.0, pos=(-150, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor='white',
        opacity=None, depth=-1.0, interpolate=True)
    ...
    for thisTrial_2 in trials_2:
        ...
        # --- Prepare to start Routine "random_routine_" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('random_routine_.started', globalClock.getTime(format='float'))
        ...
        _key_resp_2_allKeys = []
        # keep track of which components have finished
        random_routine_Components = [fixation2, line1_1, line2_2, line4_4, line3_3, probe2, key_resp_2]
        for thisComponent in random_routine_Components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            # if line1_1 is starting this frame...
            if line1_1.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
                line1_1.frameNStart = frameN  # exact frame index
                line1_1.tStart = t  # local t and not account for scr refresh
                line1_1.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(line1_1, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'line1_1.started')
                # update status
                line1_1.status = STARTED
                line1_1.setAutoDraw(True)
            
            # if line1_1 is active this frame...
            if line1_1.status == STARTED:
                # update params
                pass
        ...
        # --- Ending Routine "random_routine_" ---
        for thisComponent in random_routine_Components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('random_routine_.stopped', globalClock.getTime(format='float'))
        # check responses
        ...
        # the Routine "random_routine_" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
