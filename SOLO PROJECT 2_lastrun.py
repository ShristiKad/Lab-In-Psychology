...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\shreya kad\\Desktop\\ahmedabad university\\Lab In Psychology\\SOLO PROJECT 2\\SOLO PROJECT 2_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Instruction = visual.TextStim(win=win, name='Instruction',
        text='You will see a series of words. Try to remember as many as you can.\n',
        font='Arial',
    ...
    text = visual.TextStim(win=win, name='text',
        text='Click Space button to Start',
        font='Arial',
    ...
    # --- Initialize components for Routine "Recall_Routine" ---
    Instruction_3 = visual.TextStim(win=win, name='Instruction_3',
        text='Type all the words you can remember and press Enter when done',
        font='Arial',
        pos=(0, 0.3), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    InstructionsComponents = [Instruction, text, key_resp_2]
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('SOLO PROJECT 2.xlsx'),
    ...
    # --- Prepare to start Routine "Recall_Routine" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Recall_Routine.started', globalClock.getTime(format='float'))
    textbox.reset()
    # reset button to account for continued clicks & clear times on/off
    button.reset()
    # keep track of which components have finished
    Recall_RoutineComponents = [Instruction_3, textbox, button]
    for thisComponent in Recall_RoutineComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        # if Instruction_3 is stopping this frame...
        if Instruction_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > Instruction_3.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                Instruction_3.tStop = t  # not accounting for scr refresh
                Instruction_3.tStopRefresh = tThisFlipGlobal  # on global time
                Instruction_3.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Instruction_3.stopped')
                # update status
                Instruction_3.status = FINISHED
    ...
    # --- Ending Routine "Recall_Routine" ---
    for thisComponent in Recall_RoutineComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Recall_Routine.stopped', globalClock.getTime(format='float'))
    ...
    thisExp.nextEntry()
    # the Routine "Recall_Routine" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
